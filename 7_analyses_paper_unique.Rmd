...
plot(ggpredict(summary(model_local_adapt_01_22_FFD),terms="FFD_corr [all]")) #*
```
...
```{r}
# Something weird with surival, check later!
...
Two cases with no fruits but seeds (n_tot_seed >0)! I should probably revise all the data row by row and rerun all analyses.

```{r}
data_transplants_aster%>%
  filter(year==2023&!is.na(surv))%>%
  filter(n_tot_seed>0&nseed==0)
```
