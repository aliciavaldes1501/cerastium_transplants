...
temps_dams_sires_list<-read_csv(here("..", "cerastium_greenhouse",
                                     "data","clean","temps_dams_sires_list.csv"))
...
# NEW Appendix 1: Plot maps

Inside = mean_temp_parents, outside = temp
...
plot(get_legend(
  ggplot(data=data_transplants%>%filter(year==2022&plot=="H05"&!is.na(temp)),
       aes(x=x,y=y,color=temp,fill=mean_temp_parents,label=unique_id))+
  geom_point(size=3.5,shape=21,stroke=3)+
  scale_color_distiller(palette="YlOrRd",direction=1,
                        limits=range(data_transplants$temp, na.rm = TRUE))+
  scale_fill_distiller(palette="RdYlBu",
                       limits=range(data_transplants$mean_temp_parents,
                                    na.rm = TRUE))+
  ...
  geom_rect(aes(xmin = 0, xmax = 22, ymin = 0, ymax = 6), 
            fill = NA, color = "black", size = 2)+
  labs(x = "X coordinate (m)", y = "Y coordinate (m)", 
       color = "Soil temperature", fill = "Mid-parental temperature")+
  annotate("text", x = 1.25, y = 5.5, label = "Plot H05", size = 6,
           fontface = "bold")+
...
ggplot(data=data_transplants%>%filter(year==2022&plot=="H05"&!is.na(temp)),
  ...
  annotate("text", x = 1.25, y = 5.5, label = "Plot H05", size = 6,
           fontface = "bold")
ggsave(file="output/figures/map_H05.tiff", width=13.2,height=3.6,dpi=300,
       compression="lzw")
```
...
ggplot(data=data_transplants%>%filter(year==2022&plot=="HC1"&!is.na(temp)),
  ...
  geom_rect(aes(xmin = 0, xmax = 14, ymin = 0, ymax = 8), 
            fill = NA, color = "black", size = 2)+
  labs(x = "X coordinate (m)", y = "Y coordinate (m)", 
       color = "Soil temperature", fill = "Mid-parental temperature")+
  annotate("text", x = 1.25, y = 7.5, label = "Plot HC1", size = 6,
           fontface = "bold")
ggsave(file="output/figures/map_HC1.tiff", width=8.4,height=4.8,dpi=300,
       compression="lzw")
```
...
ggplot(data=data_transplants%>%filter(year==2022&plot=="H08"&!is.na(temp)),
  ...
  scale_x_continuous(limits = c(0, 27), breaks = seq(0, 27, by = 1))+
  geom_rect(aes(xmin = 0, xmax = 27, ymin = 0, ymax = 4), 
            fill = NA, color = "black", size = 2)+
  labs(x = "X coordinate (m)", y = "Y coordinate (m)", 
       color = "Soil temperature", fill = "Mid-parental temperature")+
  annotate("text", x = 1.25, y = 3.5, label = "Plot H08", hjust = 0.5, vjust = 1, 
           size = 6, fontface = "bold")
ggsave(file="output/figures/map_H08.tiff", width=16.2,height=2.4,dpi=300,
       compression="lzw")
```
